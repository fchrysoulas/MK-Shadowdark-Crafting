...
export function parseRecipeReference(reference, options = {}) {
  ...
  if (separatorIndex >= 0) {
    return { bookId: raw.slice(0, separatorIndex).trim() || optionBookId, id: raw.slice(separatorIndex + 2).trim() };
  }
...
export function normalizeRecipeAbilities(data = {}) {
  ...
  let abilities = [];
  if (Array.isArray(source)) abilities = source;
  ...
  return Array.from(new Set(abilities.map((ability) => String(ability || "").toLowerCase().trim()).filter((ability) => VALID_ABILITIES.includes(ability))));
}
...
export function sanitizeMaterialGroup(group = {}) {
  const rawAlternatives = Array.isArray(group.alternatives) ? group.alternatives : Array.isArray(group.items) ? group.items : [];
  const alternatives = rawAlternatives.map(sanitizeMaterialChoice).filter((material) => material.name && material.qty > 0);
  return { id: String(group.id || foundry.utils.randomID?.() || crypto.randomUUID?.() || Date.now()).trim(), alternatives };
}

export function normalizeMaterialGroups(data = {}) {
  const rawGroups = Array.isArray(data.materialGroups) ? data.materialGroups : [];
  if (rawGroups.length) return rawGroups.map(sanitizeMaterialGroup).filter((group) => group.alternatives.length > 0);
  const legacyMaterials = Array.isArray(data.materials) ? data.materials : [];
  return legacyMaterials.map((material) => ({ alternatives: [material] })).map(sanitizeMaterialGroup).filter((group) => group.alternatives.length > 0);
}
...
export function normalizeDeconstructMaterials(data = {}) {
  ...
  if (rawMaterials.length) return rawMaterials.map(sanitizeMaterialChoice).filter((material) => material.name && material.qty > 0);
  const legacyGroups = Array.isArray(data.deconstructMaterialGroups) ? data.deconstructMaterialGroups : [];
  if (legacyGroups.length) return legacyGroups.map(sanitizeMaterialGroup).flatMap((group) => group.alternatives ?? []).filter((material) => material.name && material.qty > 0);
  return [];
}
...
function buildDefaultDeconstructMaterials(recipe = {}) {
  ...
  for (const group of recipe.materialGroups ?? []) {
    ...
    materials.push({ name: material.name, uuid: material.uuid || "", type: material.type || "Basic", img: material.img || "icons/svg/item-bag.svg", qty: refundQty });
  }
...
export function sanitizeOutputItemData(data, recipe = {}) {
  ...
  const system = parsed.system && typeof parsed.system === "object" ? foundry.utils.deepClone(parsed.system) : {};
  const effects = Array.isArray(parsed.effects) ? foundry.utils.deepClone(parsed.effects).map((effect) => {
...
export function getActiveRecipeBookIds() {
  ...
  // Once modern books carry an explicit active flag, that state is authoritative
  // even when every book is false. An empty active set is a valid configuration.
...
export async function setActiveRecipeBookIds(ids = []) {
  ...
  await mutateRecipeBooks((books) => {
    for (const [id, book] of Object.entries(books)) {
      ...
      if (Boolean(book.active) === next && Object.hasOwn(book, "active")) continue;
      book.active = next;
  ...
  try {
    ...
    if (JSON.stringify(current) !== JSON.stringify(unique)) await game.settings.set(MODULE_ID, "activeRecipeBookIds", unique);
  } catch (error) {
...
export async function ensureDefaultRecipeBook() {
  ...
  let legacyIds = [];
  try { legacyIds = foundry.utils.deepClone(setting("activeRecipeBookIds") || []); } catch (_error) { legacyIds = []; }
  ...
  await mutateRecipeBooks((books) => {
    ...
    // Migrate only books that truly predate the explicit active property.
    for (const [id, book] of Object.entries(books)) {
    ...
    // A genuinely legacy/uninitialized world with no usable active marker gets
    // one sensible default. A modern world that explicitly set every book false
...
export function getRecipeEntriesForActor(_actor = null, { activeOnly = true } = {}) {
  ...
  for (const [bookId, book] of Object.entries(books)) {
    ...
    for (const rawRecipe of recipes) {
      ...
      entries.push({ id: recipe.id, uuid: recipe.id, key: makeRecipeKey(bookId, recipe.id), bookId, bookName: book.name || bookId, name: recipe.outputName, img: recipe.outputImg, type: recipe.outputType, recipe });
    }
  }
...
export async function getRecipeById(recipeId, options = {}) {
  ...
  } catch (error) { log("Recipe ID lookup failed", id, error); }
  return null;
}
...
export async function upsertRecipe(recipeData, { bookId = null } = {}) {
  ...
  await mutateRecipeBooks((books) => {
    if (!books[targetBookId]) {
      books[targetBookId] = { id: targetBookId, name: targetBookId === DEFAULT_BOOK_ID ? game.i18n.localize("MKSDC.RecipeBooks.WorldRecipesName") : targetBookId, active: true, recipes: [], createdAt: nowIso(), updatedAt: nowIso(), schemaVersion: 2 };
    }
...
export async function deleteRecipe(recipeId, options = {}) {
  ...
  if (deleted) { if (notify) ui.notifications.info(game.i18n.localize("MKSDC.Notifications.RecipeDeleted")); return true; }
...
export function checkRecipeRequirements(actor, recipe, { resourceActors = null } = {}) {
  ...
  for (const [groupIndex, group] of (recipe.materialGroups ?? []).entries()) {
    const alternatives = (group.alternatives ?? []).map((material) => {
      ...
      const row = { ...material, ownedQty, missingQty: Math.max(0, material.qty - ownedQty), sources: owned.sources, ok };
    ...
    materialGroups.push({ ...group, alternatives, selected, ok: groupOk });
  }
  if (!materialPlan.ok && (recipe.materialGroups ?? []).length) {
    ...
    missing.push(game.i18n.format("MKSDC.Requirements.MaterialGroupMissing", { materials: (failed?.alternatives ?? []).map((material) => `${material.name} x${material.qty}`).join(` ${game.i18n.localize("MKSDC.App.Or")} `) }));
  }
  let toolOk = true;
  ...
  if (setting("useGoldCost") && recipe.goldCost > 0) {
    ...
    else if (!goldOk) missing.push(game.i18n.format("MKSDC.Requirements.GoldMissing", { need: recipe.goldCost, have: goldInfo.amount }));
  }
  return { ok: materialPlan.ok && missing.length === 0, missing, materialRows, materialGroups, materialAllocation: materialPlan, toolOk, stationOk, goldOk, goldInfo, resourceActors: sources };
}

export async function getRecipeByUuid(uuid) { return getRecipeById(uuid); }
export async function getRecipeItemsForActor(actor) { return getRecipeEntriesForActor(actor); }
...
export async function createRecipeItem(recipeData, _itemData = {}) { return upsertRecipe(recipeData); }
...
export async function deleteRecipeItem(itemOrId) {
  ...
  if (!game.user.isGM) { ui.notifications.warn(game.i18n.localize("MKSDC.Notifications.GMOnly")); return false; }
  if (!itemOrId || !isRecipeItem(itemOrId)) { ui.notifications.warn(game.i18n.localize("MKSDC.Notifications.RecipeNotFound")); return false; }
  if (itemOrId.pack) { ui.notifications.warn(game.i18n.localize("MKSDC.Notifications.CompendiumRecipeReadOnly")); return false; }
...
export async function createSampleRecipes() {
  if (!game.user.isGM) { ui.notifications.warn(game.i18n.localize("MKSDC.Notifications.GMOnly")); return []; }
  await ensureDefaultRecipeBook();
  const samples = [
    { craftType: "survival", outputName: "Bone Knife", outputType: "Weapon", category: "Survival", outputImg: "icons/svg/item-bag.svg", outputQty: 1, dc: 9, abilities: ["wis", "dex"], time: "1 hour", materials: [{ name: "Bone Shards", qty: 2 }, { name: "Leather Straps", qty: 1 }], notes: "A simple camp-made knife." },
    { craftType: "armor", outputName: "Chitin Shield", outputType: "Armor", category: "Armor", outputImg: "icons/svg/item-bag.svg", outputQty: 1, dc: 12, abilities: ["int", "str"], time: "1 downtime", toolRequired: "Crafting Tools", materials: [{ name: "Chitin Plates", qty: 3 }, { name: "Leather Straps", qty: 2 }], notes: "A light shield made from hardened shell." },
    { craftType: "alchemy", outputName: "Crystal Salve", outputType: "Basic", category: "Alchemy", outputImg: "icons/svg/item-bag.svg", outputQty: 1, dc: 15, abilities: ["int"], time: "1 downtime", toolRequired: "Alchemist Tools", materials: [{ name: "Crystal Dust", qty: 1 }, { name: "Rare Herbs", qty: 2 }], notes: "A strange salve suitable for harsh wasteland campaigns." }
  ];
  const created = [];
